...
Texture2D gDiffuseMapFlare;
Texture2D gDiffuseMapFlareAlpha;
...
float4 PS(VertexOut pin, uniform int gLightCount, uniform bool gUseTexure) : SV_Target
{
	...
	float4 texColorFlare = float4(1, 1, 1, 1);
	float4 texColorFlareAlpha = float4(1, 1, 1, 1);

    if(gUseTexure)
	{
		// Sample texture.
		texColorFlare = gDiffuseMapFlare.Sample(samAnisotropic, pin.Tex);
		texColorFlareAlpha = gDiffuseMapFlareAlpha.Sample(samAnisotropic, pin.Tex);
		texColor = texColorFlare * texColorFlareAlpha;
	}
