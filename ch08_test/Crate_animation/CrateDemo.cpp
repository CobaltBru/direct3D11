  ...
  mTheta(1.3f*MathHelper::Pi), mPhi(0.4f*MathHelper::Pi), mRadius(2.5f)
{
	...
	mLastMousePos.x = 0;
	mLastMousePos.y = 0;
	for (int i = 0; i < 120; i++) mDiffuseMapSRV[i] = 0;
...
CrateApp::~CrateApp()
{
	ReleaseCOM(mBoxVB);
	ReleaseCOM(mBoxIB);
	for(int i=0;i<120;i++) ReleaseCOM(mDiffuseMapSRV[i]);
...
bool CrateApp::Init()
{
	...
	{
		wchar_t tmp[50];
		if (i + 1 < 10) swprintf_s(tmp, L"Fire/Fire00%u.dds", i+1);
		else if(i+1<100)swprintf_s(tmp, L"Fire/Fire0%u.dds", i + 1);
		else swprintf_s(tmp, L"Fire/Fire%u.dds", i + 1);

		HR(CreateDDSTextureFromFile(md3dDevice,
			tmp, &texResource, &mDiffuseMapSRV[i]))
...
void CrateApp::DrawScene()
{
    ...
    for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		Effects::BasicFX->SetDiffuseMap(mDiffuseMapSRV[mIndex]);
